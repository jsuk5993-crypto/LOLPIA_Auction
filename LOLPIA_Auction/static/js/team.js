...
function renderTeamPage(state) {
  ...
  const playerEl = document.getElementById("team-current-player");
  const posEl = document.getElementById("team-current-position");
  const bidEl = document.getElementById("team-current-bid");
  ...
  posEl.textContent = `${state.current_position || "POSITION"} / ${tier || "-"}`;
...
function renderBidRank(state) {
  const box = document.getElementById("bid-rank-list");
  box.innerHTML = "";
...
function renderMyBudget(state) {
  const myTeam = state.teams[selectedTeam];
  if (!myTeam) return;

  const remain = myTeam.budget - myTeam.spent;
  document.getElementById("my-budget").textContent = `${remain}P`;
}

function renderLogs(state) {
  const logBox = document.getElementById("team-log-list");
  logBox.innerHTML = "";

  if (!state.logs || state.logs.length === 0) {
    logBox.innerHTML = "<p>아직 로그가 없습니다.</p>";
    return;
  }
...
function renderStats(state) {
  ...
  document.getElementById("stat-sold").textContent = `${sold}명`;
  document.getElementById("stat-left").textContent = `${left}명`;
}

document.querySelectorAll(".quick-buttons button").forEach((btn) => {
  btn.addEventListener("click", () => {
    const add = Number(btn.dataset.add);
    const currentBid = latestState ? Number(latestState.current_bid || 0) : 0;
    document.getElementById("bid-amount").value = currentBid + add;
  });
});
